...
while read -r line; do
    ...
    for (( j=0; j<nr_steps; j++ )); do
        ...
        for (( k=0; k<tail_length; k++ )); do
            read curr_i curr_j <<< ${tail[$k]}

            if [[ $k -eq 0 ]]; then 
                prev_i=$head_i
                prev_j=$head_j
            else
                read prev_i prev_j <<< ${tail[$((k-1))]}
            fi

            if [[ $(are_adjacent $prev_i $prev_j $curr_i $curr_j) -eq 0 ]]; then
                read next_i next_j <<< $(tail_next_move $prev_i $prev_j $curr_i $curr_j)
                tail[$k]="$next_i $next_j"

                if [[ $k -eq $((tail_length-1)) && ${seen[$next_i,$next_j]} -ne 1 ]]; then
